...
def primitive_room():
    ...
    cylinder_template = """AttributeBegin
		Material "plastic" "rgb Kd" [{} {} {}] "rgb Ks" [{} {} {}] "float roughness" [ {} ]
		Translate {} 0 {}
		Rotate {} 0 1 0
		Rotate -90 1 0 0
    ...
    while counter < train_amount + test_amount + val_amount:
        ...
        if counter == train_amount + test_amount:
            dataset = "val"
            per_dataset_counter = 0
        back_wall_values = u.generate_random_list(0, 1, 7)
        left_wall_values = u.generate_random_list(0, 1, 7)
        right_wall_values = u.generate_random_list(0, 1, 7)
        front_wall_values = u.generate_random_list(0, 1, 7)
        world = u.emptyGlossyPrimitiveRoom(back_wall_values, right_wall_values, front_wall_values, left_wall_values)
...
def add_object(object_template, location, rotation, world):
    kd = u.generate_random_list(0, 1, 3)
    ks = u.generate_random_list(0, 1, 3)
    roughness = u.generate_random_list(0, 1, 1)

    world = world + '\n' + object_template.format(kd[0], kd[1], kd[2], ks[0], ks[1], ks[2], roughness[0], location[0], location[1], rotation)
    return world
...
def generate_data(dataset, camera, world, name):
    ...
    albedo2_name = "{}_albedo2_{}".format(dataset, name)
    reflectalbedo2_name = "{}_reflectalbedo2_{}".format(dataset, name)

    specalbedo_name = "{}_specalbedo_{}".format(dataset, name)
    specalbedo2_name = "{}_specalbedo2_{}".format(dataset, name)
    reflectspecalbedo2_name = "{}_reflectspecalbedo2_{}".format(dataset, name)

    normal2_name = "{}_normal2_{}".format(dataset, name)
    reflectnormal2_name = "{}_normal2_{}".format(dataset, name)
    ...
    direct_scene_file_content = u.makePBRT(u.directIlluminationIntegrator(), u.stratifiedSampler(), u.triangleFilter(), u.imageFilm(direct_name + ".exr"), camera, world)
    ...
    specalbedo_scene_file_content = u.makePBRT(u.albedoIntegrator(), u.stratifiedSampler(), u.triangleFilter(), u.imageFilm(albedo_name + ".exr"), camera, world)

    save_scene_file(albedo_scene_file_content, "scenefiles\\primitive_room_f2\\" + albedo_name + ".pbrt")
    save_scene_file(normal_scene_file_content, "scenefiles\\primitive_room_f2\\" + normal_name + ".pbrt")
    save_scene_file(direct_scene_file_content, "scenefiles\\primitive_room_f2\\" + direct_name + ".pbrt")
    save_scene_file(depth_scene_file_content, "scenefiles\\primitive_room_f2\\" + depth_name + ".pbrt")
    save_scene_file(gt_scene_file_content, "scenefiles\\primitive_room_f2\\" + gt_name + ".pbrt")
    save_scene_file(indirect_scene_file_content, "scenefiles\\primitive_room_f2\\" + indirect_name + ".pbrt")
